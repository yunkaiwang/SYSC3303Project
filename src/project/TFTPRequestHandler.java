...
public class TFTPRequestHandler extends Thread {
	...
	/**
	 * receive data packet
	 * resend previous ack packet if time out
	 * 
	 * @param blockNumber
	 * @return TFTPDataPacket
	 * @throws IOException 
	 */
	private TFTPDataPacket receiveDataPacket(int blockNumber) throws IOException {
		int timeouts = 0;
		while (timeouts < TFTPRequestHandler.MAX_SEND_TIMES) {
			try {
				...
				if (++timeouts >= TFTPRequestHandler.MAX_SEND_TIMES) 
					throw new IOException("DATA connection timed out. ");
			}
			ThreadLog.print("Receive DATA timed out " + timeouts + " times. Try it again. ");
	...
	/**
	 * receive ack packet
	 * resend previous data packet if time out
	 * 
	 * @param blockNumber
	 * @return TFTPAckPacket
	 * @throws IOException 
	 */
	private TFTPAckPacket receiveAckPacket(int blockNumber) throws IOException {
		int timeouts = 0;
		while (timeouts < TFTPRequestHandler.MAX_SEND_TIMES) {
			try {
				...
				if (++timeouts >= TFTPRequestHandler.MAX_SEND_TIMES) 
					throw new IOException("ACK connection timed out. ");
			}
			ThreadLog.print("Receive ACK timed out " + timeouts + " times. Try it again. ");
			sendRequest(this.resendPacket);
			continue;
		}
		return null;
